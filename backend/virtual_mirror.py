...
only use attached model images.
    ...
    def generate_try_on(self, image_path: str, texture_id: str, pattern_id: str, style_config: str, closure_type: str, has_pocket: bool, extra_details: str, front_image_id: str = "custom_thoub"):
        if not self.model:
        ...
        try:
            ...
            try:
                ...
                # This often happens if safety filters block the text or if it's an image-only response
                print(f"Post-Generation Response Notice: {text_err}")
                if response and hasattr(response, 'prompt_feedback'):
                    print(f"Prompt Feedback: {response.prompt_feedback}")
                if "safety" in str(text_err).lower():
                    description = "Generation restricted by safety filters."
                else:
            ...
            if hasattr(response, 'parts'):
                for part in response.parts:
                    ...
                    if hasattr(part, 'inline_data') and part.inline_data:
                        ...
                        import base64
                        # Save the image
                        # inline_data.data is bytes (or base64 string depending on internal rep)
                        # usually it's bytes in the SDK object
                        image_data = part.inline_data.data
                        
                        # Define absolute path to frontend public
                        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
                        # Assuming structure: /backend/virtual_mirror.py -> ../frontend/public/
